...
@Component({
    selector: 'mx-ac-view-customer',
    templateUrl: './mx-view-customer.component.html',
    styleUrls: ['./mx-view-customer.component.scss'],
    standalone: false
})
export class MXViewCustomerComponent implements OnInit, OnChanges, AfterViewInit {
  workflow = true;
  custDetails: any = [];
  ...
  constructor(private router: Router, private stepper: MatStepper, private configService: MxAdmConfigService, private dialog: MatDialog, private fb: UntypedFormBuilder,
    private formatdate: MxAdmCustomDatePipe, private pubsubservice: PubsubService, private formatcurrency: MxAdmCustomCurrencyPipe,
    private cdr: ChangeDetectorRef, private localeService: LocaleService, private utilityService: UtilityService,) {
  ...
  // Onchange event for Setting review screen data
  ngOnChanges(changes: SimpleChanges) {
    // tslint:disable-next-line:forin
    for (const propName in changes) {
      const change = changes[propName];
      if (propName === 'remarks') {
        this.viewData.remarks = change.currentValue;
        this.getInit();
      }
    }
  }
  ...
  getInit() {
    ...
    if (this.reject && this.remarks) {
      this.rejectForm.get('remark').setValue(this.remarks);
  ...
  onConfigureStaticAcctDetailsData(response): void {
    ...
    if (this.viewData.recordType === 'TXN_BLACKOUT') {
      ...
      if (response.sdoObj.remarks != null && response.sdoObj.remarks != undefined) {
  ...
  onConfigureStaticCustomerAcctDetailsData(response): void {
    if (this.viewData.recordType === 'TXN_BLACKOUT') {
      this.customerMgmtReference = response.sdoObj.workflowId ? response.sdoObj.workflowId : '';
      this.workflowId = response.workflowId;
      this.custDetails = [];
  ...
  rejectsubmit() {
    const options = {
      headers: {},
      params: { action: 'REJECTED', workflowId: this.workflowId, appId: 'RB', remarks: this.remarks }
    };
    const language = this.utilityService.language || '';
    this.pubsubservice.publishEvent('ce_api_request', {
      detail: {
        ...
        data: '',
        urlParams: '',
        options: { params: { action: 'REJECTED', workflowId: this.workflowId, appId: 'RB', remarks: this.remarks }, headers: { "Accept-Language": language } },
        method: 'put',
        callback: (res) => {
          const responseData = res.detail.body;
          console.log('responseData', responseData);
  ...
  rejectremarks() {
    this.reviewScreenResponse.emit(this.viewData);
    this.stepper.next();
  }
}