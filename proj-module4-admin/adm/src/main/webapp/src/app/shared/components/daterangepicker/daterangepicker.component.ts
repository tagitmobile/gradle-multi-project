...
import * as data from './moment-data.json';
import 'moment-timezone';
// import { ConfigService } from 'mx-custom-admin-library';
...
@Component({
    selector: 'mx-ac-daterangepicker-material',
...
export class DaterangepickerComponent implements OnInit, OnDestroy {
    @HostListener('mouseenter') handleInternalClick($event) {
    ...
    zonepickerVariables: { left: any; right: any } = { left: {}, right: {} };

    daterangepicker: { start: UntypedFormControl; end: UntypedFormControl } = { start: new UntypedFormControl(), end: new UntypedFormControl() };
    fromMonthControl = new UntypedFormControl();
    fromYearControl = new UntypedFormControl();
    toMonthControl = new UntypedFormControl();
    toYearControl = new UntypedFormControl();

    applyBtn: { disabled: boolean } = { disabled: false };
    zoneData: any = (data as any).default;
    zoneArray: any[] = [];
    @Input() monthYearPicker = false;
    @Input() zonePicker = false;
    @Input() isZonePickerDisabled: any;
    @Input() defaultZone = '';
    @Input()
    ...
    @Output() startDateChanged: EventEmitter<{ startDate: any }> = new EventEmitter();
    ...
    constructor(private el: ElementRef, private _ref: ChangeDetectorRef, private _localeService: LocaleService, private configService: MxAdmConfigService) { }

    destroy$ = new Subject<void>();

    ngOnInit(): void {
        const defaultTZ = this.configService.get('timeZone');
        this.loadTzBundle().subscribe(d => {
            if (d != null && d !== undefined) {
                moment.tz.load(d);
                this.zoneArray = d.options;
                if (this.startDate && this.startDate['_z'] && this.startDate['_z']['name'] !== '') {
                    this.zonepickerVariables[SideEnum.left].zone = this.startDate['_z']['name'];
                }
                if (this.defaultZone !== '') {
                    this.setDefaultZone(this.defaultZone);
                }
                // Default TimeZone configured in development.json file
                if (defaultTZ) {
                    this.setConfigureDefaultZone(defaultTZ, d.zonesArr);
                }
            }
        });
        ...
        if (this.startDate) {
            this.renderZonePicker(SideEnum.left);
        }
        if (this.endDate) {
            this.renderZonePicker(SideEnum.right);
        }
    ...
    setStartDate(startDate): void {
        ...
        if (this.startDate && this.startDate['_z'] && this.startDate['_z']['name'] !== '') {
            this.setStartZone(this.startDate['_z']['name']);
        }
        if (!this.timePicker) {
            this.pickingDate = true;
            this.startDate = this.startDate.startOf('day');
        ...
        if (this.startDate) {
            // this.startDate.tz("America/Los_Angeles");
        }
        if (!this.isShown) {
            this.updateElement();
        }
        this.startDateChanged.emit({ startDate: this.startDate.format() });
        this.updateMonthsInView();
    }
    ...
    // Get JSON from Moment-data.json file
    loadTzBundle(): Observable<any> {
        return of(this.zoneData).pipe(delay(1));
    }
    /**
   * To render the zone picker
   */
    renderZonePicker(side: SideEnum) {
        this.zonepickerVariables[side] = {
            zone: ''
        };
    }
    /**
  * To change on Zone select box and setting zone in start Date field
  */
    zoneChanged(side: SideEnum): void {
        this.startDate.tz(this.zonepickerVariables[side].zone);
        this.setStartDate(this.startDate);
    }
    /**
  * To set the zone value in start date
  */
    setStartZone(zoneName) {
        if (zoneName) {
            this.startDate.tz(zoneName);
            this.zonepickerVariables[SideEnum.left].zone = zoneName;
        }
    }
    /**
 * To set default time zone in moment
 */
    ...
    /**
* To set configured default time zone in moment
*/
    setConfigureDefaultZone(defaultTZ, options) {
        options.forEach(z => {
            if (z.key === defaultTZ) {
                console.log(z.values[0]);
                this.setDefaultZone(z.values[0]);
            }
        });
    }
    /**
 * To change on year select box
 */
    onChangeYear(date) {
        if (this.monthYearPicker && this.fromMonthControl.touched) {
            this.onSetMonthYearDate(date);
        }
    }
    /**
 * To change on month select box
 */
    onChangeMonth(date) {
        if (this.monthYearPicker && this.fromYearControl.touched) {
            this.onSetMonthYearDate(date);
        }
    }
    /**
 * To set on Month Year in date 
 */
    onSetMonthYearDate(date) {
        this.setStartDate(date.subtract(1, 'seconds'));
        this.setEndDate(this.startDate);
        this.updateElement();
        this.clickApply();
    }
